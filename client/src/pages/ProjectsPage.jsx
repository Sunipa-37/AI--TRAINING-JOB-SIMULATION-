...
import { api } from "../api";
import MarketplaceAdCard from "../components/MarketplaceAdCard";
import ProjectCard from "../components/ProjectCard";
import { PRODUCTS } from "../data/marketplaceProducts";

// Sprinkles 1-2 marketplace product cards in among the project cards so the
// storefront feels like a natural part of the grid rather than an overlay.
function buildGridItems(projects) {
  const items = [];
  projects.forEach((p, idx) => {
    items.push(<ProjectCard key={`project-${p.id}`} project={p} />);
    if (idx === 2 && PRODUCTS[0]) {
      items.push(<MarketplaceAdCard key="marketplace-ad-1" product={PRODUCTS[2]} />);
    }
    if (idx === 6 && PRODUCTS[4]) {
      items.push(<MarketplaceAdCard key="marketplace-ad-2" product={PRODUCTS[3]} />);
    }
  });
  return items;
}
