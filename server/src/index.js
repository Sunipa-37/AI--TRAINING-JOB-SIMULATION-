...
app.post('/api/telemetry',(req,res)=>{
  try{
    const {participantId,manufacturer,model,androidVersion,sdkVersion,batteryPercentage,charging,networkType,locationPermission,latitude,longitude} = req.body;
    if(!participantId ||!sessions[participantId]) return res.status(404).json({error:"Participant not found"});
    const telemetry = {participantId,manufacturer,model,androidVersion,sdkVersion,batteryPercentage,charging,networkType,locationPermission,latitude,longitude, timestamp: new Date().toISOString()};
    sessions[participantId].telemetry = telemetry;
    sessions[participantId].lastTelemetry = telemetry;
    sessions[participantId].currentStage = "TELEMETRY_RECEIVED";
    io.emit('telemetryUpdated', telemetry);
    console.log('Telemetry', participantId, manufacturer, model);
...
app.post('/api/events',(req,res)=>{
  try{
    ...
    sessions[participantId].events.push(event);
