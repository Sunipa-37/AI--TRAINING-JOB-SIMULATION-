...
router.post("/submit", async (req, res) => {
  ...
  try {
    ...
    if (updated[0] && updated[0].user_id) {
      ...
      if (selected) {
        // Selected students get two notifications: the congratulations, and
        // a separate one pointing them to the Terms & Conditions.
        await pool.query(
          `INSERT INTO notifications (user_id, title, body) VALUES ($1,$2,$3)`,
          [
            updated[0].user_id,
            "🎉 Congratulations! You are selected.",
            `You scored ${percent}% and have been selected for "${title}". Check your email for next steps.`,
          ]
        );
        await pool.query(
          `INSERT INTO notifications (user_id, title, body) VALUES ($1,$2,$3)`,
          [
            updated[0].user_id,
            "📄 Terms & Conditions",
            `Please review the Terms & Conditions for "${title}" before you get started.`,
          ]
        );
      } else {
        await pool.query(
